# -*- coding: utf-8 -*-
"""
Created on Wed Feb 19 12:20:24 2025
...
min_ptemp = df['Potential_temp'].min(skipna=True)
max_ptemp = df['Potential_temp'].max(skipna=True)

# Create bin edges from min_ptmep to max_ptemp
bin_edges = np.linspace(min_ptemp, max_ptemp, num_bins + 1)

##--Pandas 'cut' splits ptemp data into specified number of bins--##
df['PTemp_bin'] = pd.cut(df['Potential_temp'], bins=bin_edges)
...
binned_df = df.groupby('PTemp_bin', observed=False).agg(
   ...
   ##--Aggregate data by mean, min, and max--##
    ...
    PTemp_center=('Potential_temp', 'mean')
...
##--First subplot: Absolute Temperature vs PTemp--##
##--Averaged data in each bin is plotted against bin center--##
axs[0].plot(binned_df['Temperature_avg'], binned_df['PTemp_center'], color='#1c250c', label='Absolute Temperature')
##--Range is given by filling between data minimum and maximum for each bin--##
axs[0].fill_betweenx(binned_df['PTemp_center'], binned_df['Temperature_25th'], binned_df['Temperature_75th'], color='darkolivegreen', alpha=0.4)
axs[0].fill_betweenx(binned_df['PTemp_center'], binned_df['Temperature_min'], binned_df['Temperature_max'], color='darkolivegreen', alpha=0.2)
...
##--Second subplot: RH wrt water from H2O data--##
axs[1].plot(binned_df['Rel_humidity_w_avg'], binned_df['PTemp_center'], color='darkslategray', label='Absolute Temperature')
axs[1].fill_betweenx(binned_df['PTemp_center'], binned_df['Rel_humidity_w_25th'], binned_df['Rel_humidity_w_75th'], color='cadetblue', alpha=0.7)
axs[1].fill_betweenx(binned_df['PTemp_center'], binned_df['Rel_humidity_w_min'], binned_df['Rel_humidity_w_max'], color='cadetblue', alpha=0.3)
axs[1].set_xlabel('Relative Humidity (%)')
axs[1].set_title('RH from MR WRT Water')
#axs[1].set_xlim(-45, -16)

axs[1].axhline(y=285, color='k', linestyle='--', linewidth=1)
axs[1].axhline(y=299, color='k', linestyle='--', linewidth=1)

##--Third subplot: RH wrt ice from H2O data--##
axs[2].plot(binned_df['Rel_humidity_i_avg'], binned_df['PTemp_center'], color='navy', label='Potential Temperature')
axs[2].fill_betweenx(binned_df['PTemp_center'], binned_df['Rel_humidity_i_25th'], binned_df['Rel_humidity_i_75th'], color='lightsteelblue', alpha=1)
axs[2].fill_betweenx(binned_df['PTemp_center'], binned_df['Rel_humidity_i_min'], binned_df['Rel_humidity_i_max'], color='lightsteelblue', alpha=0.4)
axs[2].set_xlabel('Relative Humidity (%)')
axs[2].set_title('RH from MR WRT Ice')

axs[2].axhline(y=285, color='k', linestyle='--', linewidth=1)
axs[2].axhline(y=299, color='k', linestyle='--', linewidth=1)

##--Fourth subplot: RH from probe--##
axs[3].plot(binned_df['Probe_humidity_avg'], binned_df['PTemp_center'], color='indigo', label='Probe RH', alpha=1)
axs[3].fill_betweenx(binned_df['PTemp_center'], binned_df['Probe_humidity_25th'], binned_df['Probe_humidity_75th'], color='rebeccapurple', alpha=0.4)
axs[3].fill_betweenx(binned_df['PTemp_center'], binned_df['Probe_humidity_min'], binned_df['Probe_humidity_max'], color='rebeccapurple', alpha=0.2)
axs[3].set_xlabel('Relative Humidity (%)')
axs[3].set_title('RH from Probe') 
