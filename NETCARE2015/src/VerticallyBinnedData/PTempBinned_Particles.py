...
for flight in flights_to_analyze: 
    ...
    binned_df = df.groupby('PTemp_bin', observed=False).agg(
       ...
       ##--Aggregate data by mean, min, and max--##
        ...
        nuc_error_75th=('nuc_error_3sigma', lambda x: x.quantile(0.75)), 
        nuc_error_90th =('nuc_error_3sigma', lambda x: x.quantile(0.90)),
    ...
    print(np.mean(binned_df['nuc_error_center']))
    print(np.mean(binned_df['nuc_error_75th']))
    print(np.mean(binned_df['nuc_error_90th']))
    #%%
    ################
    ##--PLOTTING--##
    ################
    ...
    plt.suptitle(f"Vertical Particle Profiles - {flight.replace('Flight', 'Flight ')} ({flight_date})", fontsize=16)
