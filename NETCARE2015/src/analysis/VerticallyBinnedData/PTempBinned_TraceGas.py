...
for flight in flights_to_analyze: 
    ...
    CO_CO2 = CO_conc_aligned / CO2_conc_aligned 
    ...
    binned_df = df.groupby('PTemp_bin', observed=False).agg(
       ...
       ##--Aggregate data--##
        ...
        CO_CO2_avg=('CO_CO2', 'median'),
        CO_CO2_min=('CO_CO2', 'min'),
        CO_CO2_max=('CO_CO2', 'max'),
        CO_CO2_25th=('CO_CO2', lambda x: x.quantile(0.25)),
        CO_CO2_75th=('CO_CO2', lambda x: x.quantile(0.75))
    ...
    ##--Sixth subplot: CO/CO2 ratio--##
    axs[5].plot(binned_df['CO_CO2_avg'], binned_df['PTemp_center'], color='darkgreen', label='CO/CO2')
    axs[5].fill_betweenx(binned_df['PTemp_center'], binned_df['CO_CO2_min'],
                         binned_df['CO_CO2_max'], color='seagreen', alpha=0.2)
    axs[5].fill_betweenx(binned_df['PTemp_center'], binned_df['CO_CO2_25th'],
                         binned_df['CO_CO2_75th'], color='seagreen', alpha=0.3)
    axs[5].set_xlabel('CO/CO\u2082 ratio')
    axs[5].set_title('CO/CO\u2082')
    
    axs[5].axhline(y=285, color='k', linestyle='--', linewidth=1)
    axs[5].axhline(y=299, color='k', linestyle='--', linewidth=1)
    
    ##--Use f-string to embed flight # variable in plot title--##
    plt.suptitle(f"Vertical Profiles of Trace Gases - {flight.replace('Flight', 'Flight ')} ({flight_date})", fontsize=16)
