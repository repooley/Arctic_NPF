# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 13:13:44 2026
...
##--Pull BB emissions--##
bb_emissions = xr.open_dataset(r"C:\Users\repooley\REP_PhD\Arctic_NPF\GFED5.1_monthly_2015.nc")
...
##--Use April fire data--##
bb_time = bb_conc.sel(time="2015-04-01")
...
gs = gridspec.GridSpec(nrows=4, ncols=1, height_ratios=[2, 2, 1, 1], figure=fig)

##--Create a map with polar stereographic projection in first two subplots--##
##--First column: NPF is significant--##
ax_binnedmap_sig = fig.add_subplot(gs[0, 0], projection=ccrs.NorthPolarStereo(central_longitude=-90))
ax_map_sig = fig.add_subplot(gs[1, 0], projection=ccrs.NorthPolarStereo(central_longitude=-90))
...
##--Normalize the cmap, transparent under 1 cm elev--##
norm = plt.Normalize(vmin=0.01, vmax=6000)
...
##--Set labels--##
ax_binnedmap_sig.set_title("2015", fontsize=24)
...
for flight in flights_to_analyze:
    ...
    flight_df = NETCARE[NETCARE['Flight_num'] == flight].copy()
    ...
    for file in files:
        ...
        is_significant = pd.notna(row.nuc_significant) and row.nuc_significant > 1047
           ...
           ##--Plot original (unperturbed) trajectory--##
            ...
            if traj_num ==1 and is_significant and flight in high_lat:
                ...
                ax_map.plot(lon[:index_end],
                            lat[:index_end],
                            transform=ccrs.PlateCarree(),
                            c="k", lw=1, alpha=0.75, zorder=zorder)
                ...
                ax_map.plot(lon[:index_end],
                            lat[:index_end],
                            transform=ccrs.PlateCarree(),
                            c='midnightblue', lw=1, alpha=0.75, zorder=zorder)
