...
for flight in flights_to_analyze:
    ...
    ax4.set_title(f"10-89 nm Particle Abundance - {flight.replace('Flight', 'Flight ')} ({flight_date})", fontsize=18)
    #ax4.set_ylim(0, 6250)
    ...
    CPC3_bin_counts, _, _, _ = binned_statistic_2d(CPC3_clean_df['Latitude'], 
        CPC3_clean_df['Altitude'], CPC3_clean_df['CPC3'], statistic='count', bins=[common_lat_bin_edges, common_alt_bin_edges])
     
    ##--Counts per bin for CPC10 data--##
    CPC10_bin_counts, _, _, _ = binned_statistic_2d(CPC10_clean_df['Latitude'], 
        CPC10_clean_df['Altitude'], CPC10_clean_df['CPC10'], statistic='count', bins=[common_lat_bin_edges, common_alt_bin_edges])
     
    ##--Counts per bin for N3-10 particles--##
    nuc_bin_counts, _, _, _ = binned_statistic_2d(nuc_clean_df['Latitude'], nuc_clean_df['Altitude'], 
        nuc_clean_df['nuc_particles'], statistic='count', bins=[common_lat_bin_edges, common_alt_bin_edges])
    
    ##--Counts per bin for N10-89 particles--##
    grow_bin_counts, _, _, _ = binned_statistic_2d(grow_clean_df['Latitude'], 
        grow_clean_df['Altitude'], grow_clean_df['n_10_89'], statistic='count', bins=[common_lat_bin_edges, common_alt_bin_edges])
    ...
    ##--Use pcolormesh for the plot and use viridis for values greater than 1--##
    grow_diag_plot = ax4.pcolormesh(common_lat_bin_edges, common_alt_bin_edges, grow_bin_counts.T,  # Transpose to align correctly
        shading='auto', cmap=new_cmap, vmin=1, vmax=2000)
    
    ##--Add colorbar--##
    cb4 = fig4.colorbar(grow_diag_plot, ax=ax3)
    cb4.minorticks_on()
    cb4.set_label('Number of Data Points', fontsize=12)
    
    ##--Set axis labels--##
    ax4.set_xlabel('Latitude (°)', fontsize=12)
    ax4.set_ylabel('Altitude (m)', fontsize=12)
    ax4.set_title(f"10-89 nm Particle Counts per Bin - {flight.replace('Flight', 'Flight ')} ({flight_date})")
    #ax4.set_ylim(0, 6250)
    #ax4.set_xlim(79.5, 83.7)
