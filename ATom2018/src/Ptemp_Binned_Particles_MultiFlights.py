# -*- coding: utf-8 -*-
"""
Created on Mon Feb  9 16:45:06 2026
...
for i, (flight, df) in enumerate(zip(flights_to_analyze, all_dfs)):
    ...
    axs[0].plot(binned_df["nuc_particles_center"], binned_df["ptemp_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
    
    axs[1].plot(binned_df["aitken_particles_center"], binned_df["ptemp_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
