# -*- coding: utf-8 -*-
"""
Created on Tue Feb 10 08:59:04 2026
...
for flight in flights_to_analyze:
    ...
    df = pd.DataFrame({'ptemp': potential_temp, 'temp': temperature, 'RH': RH})
...
for i, (flight, df) in enumerate(zip(flights_to_analyze, all_dfs)):
    ...
    axs[0].plot(binned_df["temp_center"], binned_df["ptemp_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
    
    axs[1].plot(binned_df["RH_center"], binned_df["ptemp_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
    
##--Subplot 1--##
axs[0].set_ylabel("Potential Temperature (K)", fontsize=16)
axs[0].set_xlabel("Temperature (K)", fontsize=14)
axs[0].set_title("Absolute Temperature", fontsize=16)
