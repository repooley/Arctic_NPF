# -*- coding: utf-8 -*-
"""
Created on Tue Feb 10 14:30:18 2026
...
for flight in flights_to_analyze:
    ...
    ##--Place in dataframe--##
    df = pd.DataFrame({'altitude': altitude, 'O3': O3, 'NO': NO, 'NO2': NO2, 'SO2': SO2, 'HNO3': HNO3, 'C2H5OOH': C2H5OOH})
    
    ##--Drop rows where the altitude is >7000 m for comparison to other campaigns--##
    df = df[df['altitude']<7000]
...
for i, (flight, df) in enumerate(zip(flights_to_analyze, all_dfs)):
    ...
    axs[0].plot(binned_df["O3_center"], binned_df["altitude_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
    
    axs[1].plot(binned_df["NO_center"], binned_df["altitude_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
    
    axs[2].plot(binned_df["NO2_center"], binned_df["altitude_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
    
    axs[3].plot(binned_df["SO2_center"], binned_df["altitude_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
    
    axs[4].plot(binned_df["HNO3_center"], binned_df["altitude_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
    
    axs[5].plot(binned_df["C2H5OOH_center"], binned_df["altitude_center"],
                color=colors[i], label=f'{flight} ({flight_date})')
