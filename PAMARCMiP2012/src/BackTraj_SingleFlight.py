# -*- coding: utf-8 -*-
"""
Created on Thu Feb 26 09:48:06 2026
...
hysplit = r"C:\Users\repooley\REP_PhD\Arctic_NPF\PAMARCMiP2012\data\raw\HYSPLIT"

##--define the parent directory--##
parent_output_dir = r"C:\Users\repooley\REP_PhD\Arctic_NPF\PAMARCMiP2012\data\processed"

PAMARCMiP = pd.read_csv(r"C:\Users\repooley\REP_PhD\Arctic_NPF\PAMARCMiP2012\data\raw\PAMARCMiP.csv")

###########################################
##--Flights to be analyzed individually--##
###########################################

##--Flights to analyze - flights 1-9--##
flights_to_analyze = ["Flight1", "Flight2"]
...
for flight in flights_to_analyze: 
    ...
    single_flight = PAMARCMiP[PAMARCMiP['Flight_num'] == flight]
    ...
    PAMARCMiP_subset = single_flight[single_flight['Time_start'].isin(UTCs)]
    ...
    ##--Set labels--##
    ax_map_sig.set_title("$N_{10-60}$ Event", fontsize=14)
    ax_map_nonsig.set_title("No Significant $N_{10-60}$", fontsize=14)
    ax_time_sig.set_ylabel("Meters Above Sea Level", fontsize=12)
    #ax_time_nonsig.set_ylabel("Meters Above Ground Level", fontsize=12)
    
    fig.suptitle(f"PAMARCMiP {flight.replace('Flight', 'Flight ')} ({flight_date}) HYSPLIT Back Trajectories", 
                     fontsize=16, y=htitle)
    
    ##--Set axes limits--##
    #ax_time_sig.set_ylim(-250, 11000)
    #ax_time_nonsig.set_ylim(-250, 11000)
    ...
    for i, (file, row) in enumerate(zip(sorted(os.listdir(flight_directory)), PAMARCMiP_subset.itertuples(index=False))):
    
        ##--Determine which axis to use (Aitken vs non-Aitken)--##
        is_significant = pd.notna(row.aitken_significant)
    ...
    folder_path = os.path.join(parent_output_dir, "HYSPLIT", f"{flight}")
