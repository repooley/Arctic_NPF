# -*- coding: utf-8 -*-
"""
Created on Wed Jan 28 08:55:06 2026
...
for flight in flights_to_analyze:
    ...
    ##--Add Aitken mode to df--##
    condensation_sink['Aitken'] = n_10_60_df['35']
    ...
    ##--Add to condensation sink dataframe--##
    condensation_sink['LoD'] = aitken_error_3sigma['aitken_error_3sigma']
    
##--Concatenate the resulting list of dataframes into single dataframe--##
condensation_sinks = pd.concat([condensation_sink])
...
##--High lat flights--##
condensation_aitken = condensation_sinks['Condensation_Sink'][condensation_sinks['Aitken']
                                           > condensation_sinks['LoD']]
condensation_noaitken = condensation_sinks['Condensation_Sink'][condensation_sinks['Aitken']
                                           <= condensation_sinks['LoD']]

##--Final dataframes to feed to the violin plots--##
##--Drop index to prevent reindexing issues--##
condensation = pd.DataFrame({
    'Aitken': condensation_aitken.reset_index(drop=True),
    'No Aitken': condensation_noaitken.reset_index(drop=True)
})
...
conden_aitken_array = condensation_aitken.dropna().to_numpy() # data should be in a list or array
conden_noaitken_array = condensation_noaitken.dropna().to_numpy()

U_conden, p_conden = mannwhitneyu(conden_aitken_array, conden_noaitken_array)
...
sns.violinplot(data=condensation, order = ['Aitken', 'No Aitken'], 
...
plt.suptitle('PAMARCMiP Condensation Sink', fontsize=12, y=0.92)
...
plt.text(0.28, 0.125, "N={}".format(conden_aitken_count), transform=fig.transFigure, fontsize=10, color='dimgrey')
plt.text(0.66, 0.125, "N={}".format(conden_noaitken_count), transform=fig.transFigure, fontsize=10, color='dimgrey')
