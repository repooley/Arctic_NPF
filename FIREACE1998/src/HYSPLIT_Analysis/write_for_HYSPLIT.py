...
for flight in flights_to_analyze:
    ...
    # Ensure Time is zero-padded HHMMSS
    time_str = data['Time'].astype(str).str.zfill(6)
    
    time_sec = (
    time_str.str.slice(0, 2).astype(int) * 3600
    + time_str.str.slice(2, 4).astype(int) * 60
    + time_str.str.slice(4, 6).astype(int)
    )
    
    # detect midnight rollover
    day_offset = (time_sec.diff() < 0).cumsum()
    
    # make time continuous across midnight (CRITICAL)
    time_continuous = time_sec + day_offset * 86400
    
    # vectorized datetime construction (AIMMS-style)
    ...
    ##--Also add a column for Aitken mode significance--##
    aitken_error_3sigma = 
    
    aitken_significant = np.where(aitken_particles > aitken_error_3sigma, aitken_particles, np.nan)
...
##--Write netCDF file--##
ds = xr.Dataset.from_dataframe(FIREACE)

##--Write and save parquet file--##
ds.to_netcdf(r"C:\Users\repooley\REP_PhD\Arctic_NPF\FIREACE1998\data\raw\FIREACE.nc")
