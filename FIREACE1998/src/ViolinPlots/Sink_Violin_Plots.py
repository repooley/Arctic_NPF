# -*- coding: utf-8 -*-
"""
Created on Fri Jan 23 09:30:18 2026
...
for flight in flights_to_analyze:
    ...
    condensation_npf = condensation_sink.loc[condensation_sink['nucleating'] > LoD, 'Condensation_Sink']
    condensation_nonpf = condensation_sink.loc[condensation_sink['nucleating'] <= LoD, 'Condensation_Sink']
    
    coagulation_npf = coagulation_sink.loc[coagulation_sink['nucleating'] > LoD, 'Coagulation']
    coagulation_nonpf = coagulation_sink.loc[coagulation_sink['nucleating'] <= LoD, 'Coagulation']
    ...
    plt.suptitle(f"Condensation Sink - FIRE-ACE {flight.replace('Flight', 'Flight ')} ({flight_date})", fontsize=14, y=0.92)
    ...
    ax.tick_params(axis='x', which='both', labelsize=14, top=False, labeltop=False)
    ax.tick_params(axis='y', which='both', labelsize=14, top=False, labeltop=False)
    ...
    plt.suptitle(f"Coagulation Sink - FIRE-ACE {flight.replace('Flight', 'Flight ')} ({flight_date})", fontsize=14, y=0.95)
