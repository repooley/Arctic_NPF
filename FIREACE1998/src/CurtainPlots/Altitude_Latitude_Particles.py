...
for flight in flights_to_analyze:
    ...
    n_10_130_df = pd.DataFrame({'70': n_10_130, 'Time':averaged_data['Time']}).set_index('Time')
    ...
    all_bins_aligned = pd.concat([n_10_130_df, PCASP_STP], axis=1)
    ...
    ##--10-130 nm, Aitken--##
    grow_df = pd.DataFrame({'Altitude': averaged_data['Altitude'], 'Latitude': averaged_data['Latitude'], 'grow_particles': n_10_130})
    ...
    CPC3_bin_medians, _, _, _ = binned_statistic_2d(CPC3_clean_df['Latitude'], 
        CPC3_clean_df['Altitude'], CPC3_clean_df['CPC3'], statistic='median', 
        bins=[common_lat_bin_edges, common_alt_bin_edges])
    
    ##--CPC10--##
    CPC10_bin_medians, _, _, _ = binned_statistic_2d(CPC10_clean_df['Latitude'], 
        CPC10_clean_df['Altitude'], CPC10_clean_df['CPC10'], statistic='median', 
        bins=[common_lat_bin_edges, common_alt_bin_edges])
    
    ##--N(2.5-10)--##
    nuc_bin_medians, _, _, _ = binned_statistic_2d(nuc_clean_df['Latitude'], 
        nuc_clean_df['Altitude'], nuc_clean_df['nuc_particles'], statistic='median', 
        bins=[common_lat_bin_edges, common_alt_bin_edges])
    
    ##--N(10-130)--##
    grow_bin_medians, _, _, _ = binned_statistic_2d(grow_clean_df['Latitude'],
        grow_clean_df['Altitude'], grow_clean_df['grow_particles'], statistic='median', 
        bins=[common_lat_bin_edges, common_alt_bin_edges])
    ...
    cb4.set_label('10-60 nm Particles $(Counts/cm^{3})$', fontsize=16)
    
    ##--Set axis labels--##
    ax4.set_xlabel('Latitude (°)', fontsize=16)
    ax4.set_ylabel('Altitude (m)', fontsize=16)
    ax4.tick_params(axis='both', labelsize=16)
    ax4.set_title(f"10-60 nm Particle Abundance - {flight.replace('Flight', 'Flight ')} ({flight_date})", fontsize=18)
    #ax4.set_ylim(0, 6250)
    ...
    ##--Use pcolormesh for the plot, set minimum value for viridis colors as 1--##
    Count_plot = ax5.pcolormesh(common_lat_bin_edges, common_alt_bin_edges, Count_bin_medians.T,  # Transpose to align correctly
        shading='auto', cmap=new_cmap, vmin=0, vmax=5000)
    
    ##--Add colorbar--##
    cb5 = fig5.colorbar(Count_plot, ax=ax5)
    cb5.minorticks_on()
    cb5.ax.tick_params(labelsize=16)
    cb5.set_label('Total Count $(STP/cm^{3})$', fontsize=16)
    
    ##--Set axis labels--##
    ax5.set_xlabel('Latitude (°)', fontsize=16)
    ax5.set_ylabel('Altitude (m)', fontsize=16)
    ax5.tick_params(axis='both', labelsize=16)
    ax5.set_title(f"Total Particle Count - {flight.replace('Flight', 'Flight ')} ({flight_date})", fontsize=18)
    #ax5.set_ylim(245, 301)
