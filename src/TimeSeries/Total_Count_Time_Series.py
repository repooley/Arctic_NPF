...
n_10_85 = (CPC10_counts['Counts'] - UHSAS_total_aligned['Total_count'])

##--Change calculated particle counts less than zero to NaN--##
n_10_85 = np.where(n_10_85 >= 0, n_10_85, np.nan)

##--Put N(10-85) bin center in a df--##
n_10_85_center = pd.DataFrame([38])

##--Convert n_10_85 to a df--##
n_10_85 = pd.DataFrame({'38': n_10_85, 'time':aimms_time}).set_index('time')
...
##--Define bin edges--##
##--First bin: 10–85 nm (between CPC10 and UHSAS)--##
low_bounds = [10]
high_bounds = [85]

##--Add UHSAS bounds to list--##
low_bounds.extend(UHSAS_lower_bound.tolist())
high_bounds.extend(UHSAS_upper_bound.tolist())

##--Add OPC bounds to list--##
low_bounds.extend(OPC_lower_bound.tolist())
high_bounds.extend(OPC_upper_bound.tolist())

##--Combined bin edges: all lower bounds + final upper bound--##
combined_bin_edges = low_bounds + [high_bounds[-1]]

##--Convert to numpy array for later use--##
combined_bin_edges = np.array(combined_bin_edges)
...
##--Smooth N(2.5-10) trace with rolling average (window size in seconds)--##
nuc_particles_smooth = pd.Series(nuc_particles).rolling(window=30, center=True, min_periods=1).mean()

##--Concatenate bin centers and reindex--##
bin_centers = pd.concat([n_10_85_center, UHSAS_bin_center, OPC_bin_center], axis=0).reset_index(drop=True)

##--Create df containing UHSAS and OPC columns--##
optical_bins_aligned = pd.concat([n_10_85, UHSAS_bins_aligned, OPC_conc_STP], axis=1)
...
time_mesh, diameter_mesh = np.meshgrid(time_edges, combined_bin_edges)

##--Use pcolormesh which is more flexible than imshow--## 
fig, ax1 = plt.subplots(figsize=(12, 8))
c = ax1.pcolormesh(time_mesh, diameter_mesh, optical_conc, shading='auto', cmap='viridis')
