# -*- coding: utf-8 -*-
"""
Created on Tue Apr 15 11:27:38 2025

@author: repooley
"""
##--Compare condensation sink calcs--##
##--Williamson method 1 order of mag too high currently--##
##--Trad method 1-2 orders too LOW but correct in curtain plot code--##
...
UHSAS_total_num = pd.DataFrame({'Time' : UHSAS_time, 'Total_count': UHSAS_total_num})
...
################################################
##--Condensation sink using coagulation calc--##
################################################

##--Plug in a sulfuric acid molecule for the small particle--##
...
##--Vol spherical sulfuric acid molecule--##
sa_vol = (4/3) * np.pi * (sa_diam / 2) ** 3 #m^3
...
##--Mean molecule speed--##
sa_speed = ((8 * k * temperature_series) / (np.pi * sa_mass))**(1/2)

##--Collision cross section, assumes collision with equal size molecule--##
sa_collision_cross = np.pi * (sa_diam )**2 # m^2

##--Estimate of mean free path against air for use in slip correction--##
sa_mfp_estimate = 1/(np.pi * (1 + z_sa)**(1/2) * Nair * sigma_sa**2) # m

##--Knudsen number--##
sa_knudsen = sa_mfp_estimate / (sa_diam/2) # unitless 

##--Cunningham slip correction--##
sa_slip = 1 + 2 * sa_knudsen * (2.514 + 0.800 * np.exp(-0.550 / sa_knudsen))

##--Diffusivity--##
sa_diffusivity = (k * temperature_series * sa_slip / (3 * np.pi * dynam_viscosity * sa_diam)) # m^2/s

##--Mean free path--##
sa_mfp = ((8 * sa_diffusivity) / (np.pi * sa_speed)) # m 

##--Calculate g coefficient--##
sa_g = (2**(1/2) / (3 * sa_diam * sa_mfp)) * ((sa_diam + sa_mfp)**3 - (sa_diam**2 + sa_mfp**2)**(3/2)) - sa_diam

##--Loop through dfs in diameter_dfs and calculate needed variables for each bin--##
sa_conden_sum = pd.Series(0, index=aimms_time)  # Temporary storage for summing CS contributions
...
for diameter, df in diameter_dfs.items():
    ...
    ##--Compute the coagulation kernel per bin--##
    df['Coagulation_kernel'] = (2 * np.pi * (sa_diffusivity + diffusivity) * (sa_diam + mean_diameter) * 
                          ((sa_diam + mean_diameter) / (sa_diam + mean_diameter + 2*(sa_g**2 + g**2)**(1/2))
                           + (8 * (sa_diffusivity + diffusivity)) / ((sa_speed**2 + speed**2)**(1/2) * 
                                                                      (sa_diam + mean_diameter)))**-1)
    
    ##--Calculate coagulation by multiplying kernel by particle concentration per bin--##
    df['Coagulation'] = df['Coagulation_kernel'] * df['Particle_concentration'] # s^-1
    
    ##--Sum the coagulation kernels across bins--##
    sa_conden_sum += df['Coagulation']  # Accumulate per bin

##--Create dataframe and populate with coag_sum series--##    
sa_condensation_sink = pd.DataFrame(index=aimms_time, columns=['Coagulation'])
sa_condensation_sink['Coagulation'] = sa_conden_sum
...
##--Repeat for condensation sink using coagulation calculation--##
sa_condensation_sink['Condensation_Sink_smoothed'] = sa_condensation_sink['Coagulation'].rolling(window=window_size, min_periods=1, center=True).mean()

# Plot the condensation sink on the same axis as the coagulation sink
ax2.plot(sa_condensation_sink.index, sa_condensation_sink['Condensation_Sink_smoothed'], color='blue', label='Condensation Sink - Coag method', linewidth=2)
