# -*- coding: utf-8 -*-
"""
Created on Tue Apr 29 11:20:45 2025
...
for flight in flights_to_analyze:
    ...
    cond_sink = pd.Series(0, index=aimms_time)
    ...
    for diameter, df in diameter_dfs.items():
        ...
        cond_contrib = (2 * np.pi * df['Diffusion_coefficient'] * df['CS_contribution']).fillna(0)
    ...
    coag_sink = pd.Series(0, index=aimms_time)
...
condensation = pd.DataFrame({
    'High_NPF': condensation_highlat_npf.reset_index(drop=True),
    'Low_NPF': condensation_lowlat_npf.reset_index(drop=True),
    'High_NoNPF': condensation_highlat_nonpf.reset_index(drop=True),
    'Low_NoNPF': condensation_lowlat_nonpf.reset_index(drop=True)
})

coagulation = pd.DataFrame({
    'High_NPF': coagulation_highlat_npf.reset_index(drop=True),
    'Low_NPF': coagulation_lowlat_npf.reset_index(drop=True),
    'High_NoNPF': coagulation_highlat_nonpf.reset_index(drop=True),
...
##--Counts--##
conden_hi_npf_count = len(condensation_highlat_npf)
conden_lo_npf_count = len(condensation_lowlat_npf)
conden_hi_nonpf_count = len(condensation_highlat_nonpf)
conden_lo_nonpf_count = len(condensation_lowlat_nonpf)

coag_hi_npf_count = len(coagulation_highlat_npf)
coag_lo_npf_count = len(coagulation_lowlat_npf)
...
plt.text(0.36, 0.125, "N={}".format(conden_hi_nonpf_count), transform=fig.transFigure, fontsize=10, color='dimgrey')
plt.text(0.56, 0.125, "N={}".format(conden_lo_npf_count), transform=fig.transFigure, fontsize=10, color='dimgrey')
plt.text(0.75, 0.125, "N={}".format(conden_lo_nonpf_count), transform=fig.transFigure, fontsize=10, color='dimgrey')
...
plt.text(0.36, 0.125, "N={}".format(coag_hi_nonpf_count), transform=fig.transFigure, fontsize=10, color='dimgrey')
plt.text(0.56, 0.125, "N={}".format(coag_lo_npf_count), transform=fig.transFigure, fontsize=10, color='dimgrey')
plt.text(0.75, 0.125, "N={}".format(coag_lo_nonpf_count), transform=fig.transFigure, fontsize=10, color='dimgrey')
